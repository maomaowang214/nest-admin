...
<script lang="ts" setup>
  ...
  const loadData = async () => {
    ...
    const markerValue = marker.value?.trim() || '';
    ...
    const hasMarker = !isEmpty(markerValue);
    if (hasMarker) {
  ...
  const handleDownload = async (row: API.SFileInfo) => {
    ...
    try {
      ...
      const filePath = isSearching.value && row.belongTo
        ? `${row.belongTo}/`
        : parsePath();

      const downloadUrl = await Api.netDiskManage.netDiskManageDownload({
        path: filePath,
        name: row.name,
      });
