...
/** 获取图片预览链接，适用于私有空间的图片预览 GET /api/netdisk/manage/preview */
export async function netDiskManagePreview(
  // 叠加生成的Param类型 (非body参数swagger默认没有生成对象)
  params: API.NetDiskManagePreviewParams,
  options?: RequestOptions,
) {
  return request<string>('/api/netdisk/manage/preview', {
    method: 'GET',
    params: {
      ...params,
    },
...
/** 图片代理接口，解决Mixed Content问题 GET /api/netdisk/manage/proxy */
export async function netDiskManageProxy(
  params: { url: string },
  options?: RequestOptions,
) {
  return request<string>('/api/netdisk/manage/proxy', {
    method: 'GET',
    params: {
      ...params,
    },
